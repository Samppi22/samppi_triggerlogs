RegisterNetEvent('samppi_triggerlogger:discordLog')
AddEventHandler('samppi_triggerlogger:discordLog', function(playerId, title, message, webhook, triggerName, triggerType)
...
for k in pairs(Config.Triggers) do
    if Config.Triggers[k].type == "server" then
        ...
        AddEventHandler(Config.Triggers[k].trigger, function()
            TriggerEvent('samppi_triggerlogger:discordLog', source, Config.Triggers[k].title, Config.Triggers[k].message, Config.Triggers[k].webhook, Config.Triggers[k].trigger, Config.Triggers[k].type)
        end)
    end
end
